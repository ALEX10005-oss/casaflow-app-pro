...
function Calendario() {
  ...
  const selectedGuest = selectedReservation ? guests.find((g) => g.id === selectedReservation.guest_id) ?? null : null;
  ...
  return (
    ...
    >
      <div className="mb-3 flex flex-wrap items-center gap-3 rounded-lg border bg-card px-3 py-2 text-xs">
        ...
        <span className="ml-auto rounded-md border bg-muted/40 px-2 py-1 font-medium text-foreground">Clic en reserva = ver y editar dentro del calendario · borde derecho = cambiar noches.</span>
      </div>
        ...
        <CardContent ref={scrollRef} onScroll={() => syncScroll("main")} className="h-[calc(100vh-235px)] min-h-[420px] overflow-auto p-0">
          <div style={{ minWidth: fullWidth }}>
            <div className="sticky top-0 z-50 border-b bg-card shadow-md">
              <div className="flex">
                ...
                <div className="flex" style={{ width: timelineWidth }}>
                  {monthGroups.map((m) => <div key={m.key} className="shrink-0 border-r bg-muted/50 py-2 text-xs font-semibold uppercase tracking-wide" style={{ width: m.days * DAY_WIDTH }}><span className="px-3">{m.name} {year}</span></div>)}
                </div>
              </div>
            ...
            {properties.map((property, propertyIndex) => {
              ...
              const timeline = [
                ...propertyReservations.map((r) => ({ id: `r-${r.id}`, reservationId: r.id, externalId: null as string | null, from: r.check_in, to: resizePreview?.id === r.id ? resizePreview.checkOut : r.check_out, name: guestById[r.guest_id ?? ""]?.full_name ?? r.code, channel: r.channel, kind: "reservation" as const })),
                ...propertyExternal.map((e) => ({ id: `e-${e.id}`, reservationId: null as string | null, externalId: e.id, from: e.start_date, to: e.end_date, name: e.summary || e.channel, channel: e.channel, kind: "external" as const })),
                ...propertyBlocks.map((b) => ({ id: `b-${b.id}`, reservationId: null as string | null, externalId: null as string | null, from: b.start_date, to: b.end_date, name: b.reason || "Bloqueado", channel: "", kind: "block" as const })),
              ];
              const selectedInProperty = selectedReservation?.property_id === property.id ? selectedReservation : null;

              return (
                <div key={property.id}>
                  <div className="flex border-b">
                    <div className={cn("sticky left-0 z-30 shrink-0 border-r px-3 py-2", propertyIndex % 2 === 0 ? "bg-card" : "bg-muted/40")} style={{ width: PROPERTY_WIDTH, height: ROW_HEIGHT }}><p className="truncate text-xs font-semibold">{property.name}</p><p className="truncate text-[10px] text-muted-foreground">{property.code} · {property.location}</p></div>
                    <div className="relative bg-background" style={{ width: timelineWidth, height: ROW_HEIGHT }}>
                      ...
                      {timeline.map((item) => {
                        ...
                        const labelText = item.kind === "reservation" ? `${item.name} · ${item.channel} · ${shortDate(item.from)}–${shortDate(item.to)} · ${widthDays} noches` : item.name;
                        return <div key={item.id} role={item.reservationId || item.externalId ? "button" : undefined} tabIndex={item.reservationId || item.externalId ? 0 : -1} onClick={() => { if (item.reservationId) setSelectedReservationId(item.reservationId); else if (item.externalId) setSelectedExternalId(item.externalId); }} className="absolute inset-y-[7px] z-[10] overflow-visible" style={{ left: startIndex * DAY_WIDTH + 2, width: width - 4 }} title={`${item.name} · ${shortDate(item.from)} → ${shortDate(item.to)}`}>
                          <div className={cn("absolute inset-0 flex items-center overflow-hidden rounded-full px-3 pr-14 text-left text-[11px] font-semibold text-white shadow-sm ring-1 ring-black/5", item.kind === "block" ? "bg-[#334155]" : CHANNEL_COLOR[item.channel] ?? "bg-[#FF5A5F]", item.kind === "external" && "border-2 border-dashed border-white/70", (item.reservationId || item.externalId) && "cursor-pointer hover:brightness-105", isSelected && "ring-2 ring-primary ring-offset-2")}><span className="block min-w-0 flex-1 truncate">{labelText}</span></div>
                          {reservation && canEdit && <button type="button" className="absolute -right-1 top-1/2 z-40 grid h-9 w-6 -translate-y-1/2 cursor-ew-resize place-items-center rounded-md border border-white/80 bg-foreground text-background shadow-lg" title="Arrastrar para agregar o quitar noches" onPointerDown={(e) => startResize(e, reservation)}><GripVertical className="size-4" /></button>}
                        </div>;
                      })}
                    </div>
                  </div>
                  ...
                  {selectedInProperty && (
                    <div className="flex border-b border-primary/30 bg-primary/[0.03]">
                      <div className="sticky left-0 z-30 shrink-0 border-r bg-card px-3 py-3" style={{ width: PROPERTY_WIDTH }}>
                        <p className="text-xs font-semibold text-primary">Reserva seleccionada</p>
                        <p className="mt-1 truncate text-xs">{selectedGuest?.full_name ?? selectedInProperty.code}</p>
                        <Button variant="ghost" size="sm" className="mt-2 h-7 px-2 text-xs" onClick={() => setSelectedReservationId(null)}><X className="mr-1 size-3" /> Cerrar</Button>
                      </div>
                      <div className="relative min-h-[280px]" style={{ width: timelineWidth }}>
                        <div className="sticky z-20 my-3 w-[min(1100px,calc(100vw-320px))] rounded-xl border bg-card p-4 shadow-lg" style={{ left: PROPERTY_WIDTH + 12 }}>
                          <InlineReservationEditor reservation={selectedInProperty} guest={selectedGuest} property={property} properties={properties} canEdit={canEdit} />
                        </div>
                      </div>
                    </div>
                  )}
...
function InlineReservationEditor({ reservation, guest, property, properties, canEdit }: { reservation: Reservation; guest: Guest | null; property: Property; properties: Property[]; canEdit: boolean }) {
  ...
  }, [reservation.id, reservation.updated_at, guest?.id, guest?.email, guest?.phone, guest?.full_name]);

  const set = (key: keyof typeof form, value: string) => setForm((f) => ({ ...f, [key]: value }));
  const save = async () => {
    if (!form.guest_name.trim()) return toast.error("Escribe el nombre del huésped.");
    if (!form.check_in || !form.check_out || form.check_out <= form.check_in) return toast.error("La salida debe ser posterior a la entrada.");
    const guestsCount = Number(form.guests_count);
    const total = Number(form.total_amount);
    if (!Number.isFinite(guestsCount) || guestsCount < 1) return toast.error("El número de huéspedes debe ser válido.");
    ...
    return (
      <div>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Datos de la reserva dentro del calendario</p><h3 className="text-base font-semibold">{guest?.full_name ?? reservation.code}</h3></div>
          {canEdit && <Button size="sm" onClick={() => setEditing(true)}><Pencil className="mr-1 size-4" /> Editar aquí</Button>}
        </div>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
          <Info label="Huésped" value={guest?.full_name ?? "—"} /><Info label="Propiedad" value={property.name} /><Info label="Canal / origen" value={reservation.channel} /><Info label="Código" value={reservation.code || "—"} /><Info label="Check-in" value={shortDate(reservation.check_in)} /><Info label="Check-out" value={shortDate(reservation.check_out)} /><Info label="Noches" value={String(nightsBetween(reservation.check_in, reservation.check_out))} /><Info label="Huéspedes" value={String(reservation.guests_count ?? "—")} /><Info label="Total" value={money(Number(reservation.total_amount ?? 0))} /><Info label="Estado" value={reservation.status} /><Info label="Pago" value={reservation.payment_status} /><Info label="Correo" value={guest?.email ?? "—"} /><Info label="Teléfono" value={guest?.phone ?? "—"} />
        </div>
        {reservation.notes && <div className="mt-2 rounded-md border bg-muted/20 px-3 py-2 text-sm"><span className="font-medium">Notas: </span>{reservation.notes}</div>}
        {!canEdit && <p className="mt-2 text-xs text-muted-foreground">Tu rol puede consultar esta reserva, pero no modificarla.</p>}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between gap-2"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Edición inline</p><h3 className="text-base font-semibold">Editar reserva sin salir del calendario</h3></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => { setForm(reservationForm(reservation, guest)); setEditing(false); }}>Cancelar</Button><Button size="sm" disabled={update.isPending} onClick={save}><Check className="mr-1 size-4" /> {update.isPending ? "Guardando..." : "Guardar"}</Button></div></div>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <Field label="Huésped"><input className="input-inline" value={form.guest_name} onChange={(e) => set("guest_name", e.target.value)} /></Field>
        <Field label="Correo"><input className="input-inline" type="email" value={form.guest_email} onChange={(e) => set("guest_email", e.target.value)} /></Field>
        <Field label="Teléfono"><input className="input-inline" value={form.guest_phone} onChange={(e) => set("guest_phone", e.target.value)} /></Field>
        <Field label="Propiedad"><select className="input-inline" value={form.property_id} onChange={(e) => set("property_id", e.target.value)}>{properties.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></Field>
        <Field label="Canal / origen"><input className="input-inline" value={form.channel} onChange={(e) => set("channel", e.target.value)} /></Field>
        <Field label="Código"><input className="input-inline" value={form.code} onChange={(e) => set("code", e.target.value)} /></Field>
        <Field label="Check-in"><input className="input-inline" type="date" value={form.check_in} onChange={(e) => set("check_in", e.target.value)} /></Field>
        <Field label="Check-out"><input className="input-inline" type="date" value={form.check_out} onChange={(e) => set("check_out", e.target.value)} /></Field>
        <Field label="Noches"><input className="input-inline bg-muted/50" readOnly value={String(nightsBetween(form.check_in, form.check_out))} /></Field>
        <Field label="Huéspedes"><input className="input-inline" type="number" min="1" value={form.guests_count} onChange={(e) => set("guests_count", e.target.value)} /></Field>
        <Field label="Total"><input className="input-inline" type="number" min="0" step="0.01" value={form.total_amount} onChange={(e) => set("total_amount", e.target.value)} /></Field>
        <Field label="Estado"><select className="input-inline" value={form.status} onChange={(e) => set("status", e.target.value)}><option value="confirmada">Confirmada</option><option value="pendiente">Pendiente</option><option value="in_house">Hospedado</option><option value="cancelada">Cancelada</option><option value="no_show">No show</option></select></Field>
        <Field label="Pago"><select className="input-inline" value={form.payment_status} onChange={(e) => set("payment_status", e.target.value)}><option value="pendiente">Pendiente</option><option value="registrado">Registrado</option><option value="pagado">Pagado</option><option value="parcial">Parcial</option></select></Field>
        <Field label="Notas" wide><textarea className="input-inline min-h-20 resize-y" value={form.notes} onChange={(e) => set("notes", e.target.value)} /></Field>
      </div>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-md border bg-muted/20 px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{label}</p><p className="truncate text-sm font-medium" title={value}>{value}</p></div>;
}
function Field({ label, children, wide = false }: { label: string; children: React.ReactNode; wide?: boolean }) {
  return <label className={wide ? "md:col-span-2 lg:col-span-3 xl:col-span-4" : ""}><span className="mb-1 block text-xs font-medium text-muted-foreground">{label}</span>{children}</label>;
}
